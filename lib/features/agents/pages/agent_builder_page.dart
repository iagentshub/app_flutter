...
class _AgentBuilderPageState extends State<AgentBuilderPage> {
  ...
  Widget _buildSetupPanel({required bool wide}) {
    final colors = Theme.of(context).colorScheme;
    return Material(
          ...
          children: [
            if (wide)
              Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Expanded(child: _buildSetupCopy()),
                  const SizedBox(width: 24),
                  SizedBox(width: 390, child: _buildConnectionField()),
                ],
              )
            else ...[
              _buildSetupCopy(),
  ...
  Widget _buildProgressSteps() {
    final currentStep = _agentSaved ? 2 : (_messages.isEmpty ? 0 : 1);
    final labels = [
      _tx('agents.builder_step_describe', 'Describe lo que necesitas'),
